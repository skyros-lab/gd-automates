...
function highlightInElement(el, re) {
  for (const node of Array.from(el.childNodes)) {
    if (node.nodeType === Node.TEXT_NODE && re.test(node.textContent)) {
      ...
      const frags = parts.map((part, i) => i % 2
        ? Object.assign(document.createElement('span'),
          { className: 'highlight', textContent: part })
...
function updateControlBtns() {
  ...
  [copyBtn, downloadBtn, clearBtn].forEach(b =>
    b.classList.toggle('disabled', isEmpty)
  );
}
const uiLog = (msg, lvl = 'log') => { rawUiLog(msg, lvl); updateControlBtns(); };

searchToggle.addEventListener('click', () => {
  const open = !floatingSearch.classList.contains('visible');
  floatingSearch.classList.toggle('visible', open);
  if (open) { logSearch.focus(); logSearch.select(); }
  else { logSearch.value = ''; applySearch(); }
});
closeSearch.addEventListener('click', () => {
  floatingSearch.classList.remove('visible');
  logSearch.value = ''; applySearch();
});
document.addEventListener('keydown', e => {
  if (e.ctrlKey && e.key.toLowerCase() === 'f') {
    e.preventDefault();
    floatingSearch.classList.add('visible');
    logSearch.focus(); logSearch.select();
  }
...
function applySearch() {
  ...
  if (!q) {
    ...
    searchCount.textContent = '0 résultat(s)'; return;
  }
  ...
  lines.forEach(l => {
    ...
    if (m?.length) { highlightInElement(l, reGlobal); hits += m.length; }
  });
...
copyBtn.addEventListener('click', () => {
  if (copyBtn.classList.contains('disabled') ||
    copyBtn.classList.contains('cooldown')) return;
...
downloadBtn.addEventListener('click', () => {
  if (downloadBtn.classList.contains('disabled') ||
    downloadBtn.classList.contains('cooldown')) return;
  const text = Array.from(logArea.children)
    .map(el => el.textContent.trimEnd()).join('\n');
  const save = window.electronAPI?.saveTextFile;
  (save ? save(text, 'log.txt') : new Promise((res, rej) => {
    try {
      const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = Object.assign(document.createElement('a'),
        { href: url, download: `discord-log-${Date.now()}.txt` });
      a.click(); URL.revokeObjectURL(url); res('téléchargé');
    } catch (e) { rej(e); }
  }))
  ...
  downloadBtn.innerHTML = `<svg viewBox="0 0 24 24">
     <path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>`;
  setTimeout(() => {
...
streamerToggle.addEventListener('click', () => {
  ...
  const eyeOpen = `<svg viewBox="0 0 24 24" fill="currentColor"><path
    d="M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9
       ...
       C17,19.5 21.27,16.39 23,12C21.27,7.61 17,4.5 12,4.5Z"/></svg>`;
  const eyeClosed = `
...
let tokenTimer, isPaused = false;
window.electronAPI.setPauseGetter(() => isPaused);
setModeState(false); hide(guildRow); hide(dmRow); hide(chanRow);
...
async function populateLists(token) {
  ...
  clear(guildSel);
  guilds.forEach(g =>
    guildSel.insertAdjacentHTML('beforeend',
      `<option value="${g.id}">${g.name}</option>`));
  clear(dmSel);
  dms.forEach(c => {
    const lbl = c.recipients
      ? c.recipients.map(r => r.username).join(', ')
      : (c.name || c.id);
...
async function fillChannels() {
  const token = $('#dToken').value.trim(), gid = guildSel.value;
...
$('#deleteForm').addEventListener('submit', e => {
  const dStart = $('#dateStart').value, dEnd = $('#dateEnd').value;
  const min = new Date('2000-01-01'), max = new Date('2100-12-31');
  if (dStart && (new Date(dStart) < min || new Date(dStart) > max)) { e.preventDefault(); alert('Date de début hors plage 2000-2100'); return; }
  if (dEnd && (new Date(dEnd) < min || new Date(dEnd) > max)) { e.preventDefault(); alert('Date de fin hors plage 2000-2100'); return; }
}, { once: true });
...
$('#deleteForm').addEventListener('submit', async e => {
  ...
  setRunning(true); window.electronAPI.setProcessFlag(true);
  logArea.innerHTML = ''; updateControlBtns();
  ...
  task.then(({ total }) => uiLog(
    total === 0
      ? 'Aucun message supprimé'
      : `${total} message${total > 1 ? 's' : ''} supprimé${total > 1 ? 's' : ''} ✔`,
    total === 0 ? 'warn' : 'ok'))
    .catch(err => uiLog('Le processus a correctement été stoppé.', 'ok'))
    .finally(() => {
...
$('#dStop').onclick = () => {
  ...
  try {
    ...
    if (ok) {
      isPaused = false;
      $('#dPause').className = 'pause'; $('#dPause').textContent = 'Suspendre temporairement';
      window.electronAPI.setProcessFlag(false);
